...
@Service
public class FetcherServiceImpl extends CommonServiceImpl implements IFetcherService {

	private static final String URL_PARAM = "lang=c&stype=&postchannel=0000&workyear=99&cotype=99&degreefrom=99&jobterm=99&companysize=99&providesalary=99&lonlat=0%2C0&radius=-1&ord_field=0&confirmdate=9&fromType=&dibiaoid=0&address=&line=&specialarea=00&from=&welfare=";
	...
	public FetchedResult fetchJobs(String keyword, String positionCode) throws Exception {
		...
		for (;; i++) {
			try {
				jobListUrl = "https://search.51job.com/list/" + position.getCode() + ",000000,0000,00,9,99," + keyword
						+ ",2," + i + ".html?" + URL_PARAM;
...
//		logger.info(url + "爬取成功。");
	...
	public int getTotalPage(String keyword, String positionCode) throws Exception {
		...
		int i = 1;
		String jobListUrl = "https://search.51job.com/list/" + position.getCode() + ",000000,0000,00,9,99," + keyword
				+ ",2," + i + ".html?" + URL_PARAM;
